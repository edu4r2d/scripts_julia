...
config_map = [
    ("Water/glu ext", "glu_extended", "rc_end_water", (5, 30), "A"),
    ("Water/glu comp", "glu_compact", "rc_end_water", (5, 30), "B"),
    ("Glu ext", "glu_extended", "rc_end_glu", (0, 2.0), "C"),
    ("Glu comp", "glu_compact", "rc_end_glu", (0, 2.0), "D")
]

plots = []
for (label, key, data_key, ylim, annotation) in config_map
    # Criar o gráfico base usando a paleta seaborn_colorblind6
    ...
    #annotate!([(5.0, posicao_y, text(annotation, plot_font, 30, :left, color=colors[4]))])
...
final_plot = plot(plots..., layout=layout, size=(1200, 800),
                  ylabel=L"\mathrm{Coordination~number~up~to~6.0~Å}",
...
#===
import Pkg; Pkg.activate("/home/m3g/Documentos/polyisobutileno/mddf_kb")
using Plots, LaTeXStrings, ComplexMixtures, PDBTools, EasyFit, ColorSchemes, MolSimToolkit
const CM, PDB = ComplexMixtures, PDBTools
...
plot_font = "Computer Modern"
default(fontfamily=plot_font, framestyle=:box, label=nothing, grid=true, legendfontsize=15)
colors = parse.(Colorant, ["#B91FFFFF", "#CD177FFF", "#FEC306FF", "#32D713FF", "#0072b2ff", "#d55e00ff", "#cc79a7ff", "#000000ff"])
...
for key in keys(systems)
    ...
    rc_water = ResidueContributions(sys["R_water"], select(sys["atoms"], "resname AAMD"); dmax=dmax_full, type=:coordination_number)
    sys["rc_water"] = rc_water
    sys["rc_end_water"] = [r[end] for r in rc_water.residue_contributions] # Nov sintaxe : r5 = [ r[end] for r in rc ]
...
for (label, key, data_key, ylim, annotation) in config_map
    ...
    p = plot(systems["$(key)_01"][data_key]; xlabel="Residue", ylabel="", 
             label="$(label) 1.00 M", color="#B91FFFFF", linewidth=2, xlim=(0, 100), ylim=ylim)
    plot!(systems["$(key)_75"][data_key]; label="$(label) 0.75 M", color="#FEC306FF", linewidth=2)
    plot!(systems["$(key)_05"][data_key]; label="$(label) 0.50 M", color="#32D713FF", linewidth=2, legend=:topright)

    # Calcular deslocamento ajustado para posicionar a anotação mais alta
    deslocamento = (ylim[2] - ylim[1]) * 0.9  # 70% do intervalo para subir a anotação
    posicao_y = ylim[1] + deslocamento       # Nova posição Y da anotação

    # Adicionar anotação no gráfico
    annotate!([(5.0, posicao_y, text(annotation, plot_font, 30, :left, colors[8]))])
